...
public class MCreatorKillmeplz extends Elementsmsmod.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;

	public MCreatorKillmeplz(Elementsmsmod instance) {
		super(instance, 2);
	...
	public void initElements() {
		keys = new KeyBinding("key.mcreator.killmeplz", GLFW.GLFW_KEY_1, "key.categories.misc");
	...
	public void onKeyInput(InputEvent.KeyInputEvent event) {
		if (!(Minecraft.getInstance().currentScreen instanceof ChatScreen)) {
			if (event.getKey() == keys.getKey().getKeyCode() && event.getAction() == GLFW.GLFW_PRESS) {
				msmod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage());
	...
	private static void pressAction(PlayerEntity entity) {
			...
			return;
		{
			...
			MCreatorKillmeplzOnKeyPressed.executeProcedure($_dependencies);
		}
	}
}
